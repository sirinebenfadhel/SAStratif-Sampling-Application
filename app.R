              ...
              column(
                ...
                box(
                  ...
                  selectInput("strat_var", "Variable de stratification :",
                              ...
                              selected = "Region"),
                  selectInput("aux_var", "Variable auxiliaire :",
                              choices = c("Area", "pop_block "),
                              selected = "Area"),
...
server <- function(input, output, session) {
  ...
  strat_result <- eventReactive(input$run_strat, {
    req(data(), input$n_strat, input$strat_var, input$aux_var)
    ...
    withProgress(message = 'Traitement en cours', value = 0, {
      ...
      # 3. Calcul des statistiques (version plus robuste)
      incProgress(0.3, detail = "Génération des statistiques")
      
      stats <- if (is.numeric(data()[[input$aux_var]])) {
        samp %>%
          group_by(!!sym(input$strat_var)) %>%
          summarise(
            N = n(),
  ...
  output$strat_alloc <- renderDT({
    ...
    datatable(
      ...
      options = list(
        pageLength = nrow(strat_result()$allocations),  # Affiche toutes les lignes
        scrollX = TRUE,
        paging = FALSE  # Désactive la pagination
      ),
      rownames = FALSE,
      caption = "Répartition par strate"
    ) %>%
      formatPercentage("Proportion", 2) %>%
      formatRound("Allocation", 0)
  })
  ...
  output$strat_stats <- renderDT({
    ...
    stats_data <- strat_result()$stats
    
    if (is.numeric(data()[[strat_result()$aux_var]])) {
      datatable(
        stats_data,
        options = list(
          dom = 't',
        ...
        rownames = FALSE,
        caption = "Statistiques descriptives par strate"
      ) %>%
        formatRound(c("Moyenne", "Médiane", "SD"), 2)
    } else {
      datatable(
        stats_data,
        options = list(
        ...
        rownames = FALSE,
        caption = "Répartition par catégorie"
      ) %>%
        formatPercentage("Proportion", 2)
    }
  })
